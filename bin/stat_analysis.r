...
Counts = read.table(paste(dir, "/count/output.counts", sep=""), header=T) 
...
dds$mutations <- relevel(dds$mutations, ref ="WT") #setting WT as reference for log2FC
dds <- DESeq(dds) # Levels : WT M
res <- results(dds)
res <- na.omit(res) #Removing NA values
